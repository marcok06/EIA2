...
let playerXP : number = 0 ;                             //Fehler 1                           // Stellt die gesammelte Erfahrung des Spielers dar.
...
let monsterName : string[] = ["Johnny", "Playboy", "Insta-Influencer", "Biene Maya", "Peter", "Lauch"]; // length = 6, da 6 Einträge. Von 0-5.
let suffix : string[] = [" des Durchfalls", " aus dem Gulli", " des Grauens", " mit Asthma", " mit Mundgeruch", " der Zerstuemmelung"]; // length = 6, da hier 6 Einträge sind. Von 0-5.

let monsterSrc : string[] = imagePush(); 
let monsterShield : string [] = ["Schutzpfanne", "Lederjacke", "Fahrradhelm"];       //neue Schildoptionen
...
function generateMonster()
{
    ...
    let newMonster : Monster = {                                        // Monster wird erstellt.
        ...
        //monsterMoney : 0,     Fehler Nr. 1 bei Funktionen
    };

    monsterArray.push(newMonster);                                      // Monster wird erst in diesem Schritt zu dem Array hinzugefügt 

   //console.log(monsterArray[monsterArray.length-1]); // Fehler Nr. 2 Funktionen              // Man kann nur auf Array-Teile zugreifen, welche definiert sind. -1 ist nicht definitiert (und wird es auch nie sein).
...
function monsterGenerateHTML()
{
    ...
    document.getElementById("monsterHoldingCell").appendChild(holdingDiv);   //Fehler Nr. 3 bei Funktionen      // Das HTML-Element muss erst noch zu einem Objekt hinzugefügt werden, in diesem Fall mit der id "monsterHoldingCell"
    ...
    monsterSrc.setAttribute("src", "imgs/"+ monsterArray[monsterArray.length - 1].monsterImage);                 // Der Pfad für das Bild muss über setAttribute festgelegt werden. Der Bildpfad kann natürlich auch anders aussehen.
    ...
    monsterShield.innerHTML = "Rüstung: "+ monsterArray[monsterArray.length - 1].monsterShield;
